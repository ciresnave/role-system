...
fn test_permission_edge_cases() {
    ...
    let result = std::panic::catch_unwind(|| Permission::new("", "documents"));
    assert!(result.is_err(), "Empty action should panic");

    let result = std::panic::catch_unwind(|| Permission::new("read", ""));
    ...
    assert!(
        result.is_err(),
        "Whitespace-only resource type should panic"
    );
}
...
fn test_permission_parsing_security() {
    ...
    let malformed_permissions = vec![
        ...
        "action:",
        ":resource",
        "action:resource:extra:parts",      // 4-part should fail
        "action:resource:extra:parts:more", // 5-part should fail
        " : ",
        "\n:\t",
        "action\u{0000}:resource",
        "action:resource\u{0000}",
        "action:resource:\u{0000}", // null in instance
        "action:resource:",         // empty instance
        ":resource:instance",       // empty action
        "action::instance",         // empty resource
    ];
    ...
    let valid_permissions = vec![
        ...
        "read:documents",
        "write:files",
        "read:documents:doc123", // 3-part permission
        "write:files:file456",   // 3-part permission
        "*:*",                   // wildcard
        "admin:*:*",             // wildcard with instance
    ];
...
fn test_unicode_and_special_characters() {
    ...
    // Test that dangerous special characters are properly rejected
    ...
    for dangerous in dangerous_chars {
        // These should panic due to validation - which is the secure behavior
        let result = std::panic::catch_unwind(|| {
            Permission::new(
                format!("action_{}", dangerous),
                format!("resource_{}", dangerous),
            )
        });
        assert!(
            result.is_err(),
    ...
    for safe in safe_chars {
        let permission = Permission::new(format!("action{}", safe), format!("resource{}", safe));
        assert!(permission.matches(&format!("action{}", safe), &format!("resource{}", safe)));
    }
}
...
fn test_conditional_permission_context_manipulation() {
    ...
    for attempt in manipulation_attempts {
        assert!(
            !permission.is_granted("access", "vault", &attempt),
            "Should reject context manipulation: {:?}",
            attempt
        );
    }
    ...
    assert!(
        permission.is_granted("access", "vault", &context_with_extra_keys),
