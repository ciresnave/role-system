...
impl Resource {
    ...
    pub fn matches_pattern(&self, pattern: &str) -> bool {
        ...
        if let Some(type_prefix) = pattern.strip_suffix("/*") {
            if self.resource_type == type_prefix {
                return true;
            }
        }
        ...
        if let Some(resource_path) = &self.path {
            if self.matches_path_pattern(resource_path, pattern) {
                return true;
            }
        }
...
impl std::fmt::Display for Resource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match (&self.name, &self.path) {
            (Some(name), Some(path)) => write!(f, "{} ({}:{} at {})", name, self.resource_type, self.id, path),
...
mod tests {
    ...
    fn test_resource_path_operations() {
        ...
        assert_eq!(resource.parent_path(), Some("/projects/web-app/docs".to_string()));
    }
